# app.py — ADI Builder: Lesson Activities & Questions (Word + Moodle)
# Run locally:
#   pip install -r requirements.txt
#   streamlit run app.py
#
# Put your logo at repo root as: logo.png (optional)
...
input, textarea, select, .stTextArea textarea {{
...
div.stButton>button {{
  ...
  border-radius: var(--radius-pill); padding: .6rem 1rem; font-weight:600;
  box-shadow: 0 6px 16px rgba(31,76,44,.20); transition: all .2s;
}}
div.stButton>button:hover {{
  filter: brightness(.98); box-shadow: 0 0 0 3px rgba(200,168,90,.40);
}}

.btn-gold button {{
  background: var(--adi-gold) !important; color: #1f2a1f !important;
  box-shadow: 0 6px 16px rgba(200,168,90,.32) !important;
}}

.pill {{
  display:inline-flex; align-items:center; justify-content:center;
  padding: 6px 12px; border-radius: 999px; border: 1px solid var(--border);
  background: #f6f8f6; color: #1f2a1f; font-weight:600; cursor:pointer;
}}
.pill:hover {{ background: #eef5ef; }}
.pill.active {{ background: var(--adi-green); color:#fff; border-color: var(--adi-green-600); }}

.small-pills .pill {{
  padding: 4px 10px; font-size: 12px; margin: 2px 6px 2px 0;
}}
...
.upload-hint .icon {{
  width:28px; height:28px; border-radius:7px; background:var(--adi-green);
...
# -----------------------------------------------------------------------------
# MCQ generation (rule-based scaffold)
# -----------------------------------------------------------------------------
def generate_mcq_blocks(topic: str, source: str, num_blocks: int, week: int) -> pd.DataFrame:
    """
    Creates num_blocks * 3 MCQs (each block has 3: Low/Medium/High).
    This is a deterministic scaffold so users can edit in the UI.
    """
...
def mcq_to_docx(df: pd.DataFrame, topic: str) -> bytes:
    ...
    doc.add_paragraph("Each block: Low → Medium → High").italic = True
...
def mcq_to_gift(df: pd.DataFrame, topic: str) -> bytes:
    """
    Moodle GIFT export. We assume single-correct MCQs.
...
with tab[0]:
    ...
    with left:
        ...
        up_file = st.file_uploader(" ", type=["pdf","docx","pptx"], label_visibility="collapsed")
        ...
        for i,c in enumerate(lcols, start=1):
            with c:
                active = " active" if st.session_state.lesson==i else ""
