...
/* FORCE green dashed border around the uploader (cover known DOM variants) */
...
/* Download panel: obvious but tidy */
.download-panel{{
...
def parse_upload_cached(file_bytes:bytes, filetype:str, deep:bool):
    try:
        if filetype=="pdf":
            ...
            page_total = len(doc)
            max_pages  = page_total if deep else min(10, page_total)  # quick = first 10 pages
...
def main():
    ...
    with st.sidebar:
        ...
        # visible status line directly below uploader
    ...
    # Parse upload once, then show single quiet status
    source_text = ""
