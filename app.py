# (Short) placeholder: bring your current app.py here.
# Keeping small because you already have app.py in your repo.
# If needed, copy the Gradio app.py from the earlier ZIP.
...
[data-testid="stDataEditor"] thead th {{
  position: sticky; top: 0; background: #ffffff; z-index: 2; box-shadow: 0 1px 0 rgba(0,0,0,.06);
}}
</style>
...
def weighted_bloom_sequence(selected:str, n:int, rng:random.Random)->List[str]:
...
def export_docx(df:pd.DataFrame, activities:List[str], lesson:int, week:int)->Optional[bytes]:
    """Legacy consolidated export (still available but not used on the new buttons)."""
    try:
        from docx import Document
    except Exception:
        return None
...
def export_activity_sheet_docx(activities:List[str], lesson:int, week:int, title:str="Activity Sheet")->Optional[bytes]:
    ...
    doc.add_paragraph(f"Lesson {lesson} · Week {week}")
    p = doc.add_paragraph(); p.add_run("Student name: ").bold = True; p.add_run("_______________________________    ")
    p.add_run("ID: ").bold = True; p.add_run("______________")
    doc.add_paragraph(" ")
    if activities:
        doc.add_heading("Activities", level=2)
...
def export_answer_key_docx(df:pd.DataFrame, lesson:int, week:int, title:str="Answer Key")->Optional[bytes]:
    ...
    hdr = tbl.rows[0].cells; hdr[0].text = "Q#"; hdr[1].text = "Answer"
    for _, r in df.iterrows():
        row = tbl.add_row().cells
        row[0].text = str(r.get("Q#","")); row[1].text = str(r.get("Answer",""))
    bio = io.BytesIO(); doc.save(bio); return bio.getvalue()
...
with tabs[1]:
    ...
    with col_right:
        ...
        st.session_state.blooms = blooms; st.session_state.count_auto = count
    ...
    if 5 <= len(st.session_state.verbs) <= 10: st.success("Verb count looks good ✅")
    else: st.warning(f"Select between 5 and 10 verbs. Currently: {len(st.session_state.verbs)}")
...
with tabs[2]:
    ...
    with g4:
        ...
        if st.button("⚡ Auto-fill MCQs"):
            st.session_state.mcq_df = offline_mcqs_varied(src,
                st.session_state.get('blooms', ["Understand"]*8),
                st.session_state.verbs,
                len(st.session_state.get('blooms', [])) or 8,
                st.session_state.get('lesson',1),
                st.session_state.get('week',1)
            )
    ...
    st.session_state["acts_textarea"] = st.text_area("One per line", key="acts_textarea", height=140, value="\n".join(st.session_state.get("activities", [])))
    ...
    if missing: st.caption(f"⚠️ Incomplete rows: {sorted(set(missing))}. Add a question and all options.")
    else: st.caption("✅ Table looks good.")
...
with tabs[3]:
    ...
    with c1:
        ...
        st.download_button("⬇️ Activity Sheet (.docx)", data=(act_bytes or b"placeholder"),
    ...
    with c2:
        ...
        st.download_button("⬇️ MCQ Paper (.docx)", data=(mcq_bytes or b"placeholder"),
    ...
    with c3:
        ...
        st.download_button("⬇️ Answer Key (.docx)", data=(key_bytes or b"placeholder"),
                           file_name=base + "_AnswerKey.docx",
                           mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                           disabled=not docx_available or (df is None or df.empty))
    with c4:
        gift_payload = to_gift(df) if (df is not None and not df.empty) else ""
        st.download_button("⬇️ Moodle GIFT (.gift)", data=(gift_payload or "").encode("utf-8"),
                           file_name=base + ".gift", mime="text/plain",
                           disabled=not bool(gift_payload))

# =========================
# Revision helpers (offline)
# =========================
STOPWORDS = set("a an and are as at be by for from has have if in into is it its of on or such that the their then there these this those to was were will with within without over under about across".split())
...
def summarize_sentences(text:str, k:int=8)->List[str]:
...
def split_text_by_week_markers(text:str)->dict:
...
def collect_weeks_text(full_text:str, start:int, end:int)->str:
    if start>end: start,end = end,start
    spans = split_text_by_week_markers(full_text)
    if spans:
        chunks = [spans[w] for w in range(start, end+1) if w in spans]
        return "\n".join(chunks)
    total_weeks = 14; n = len(full_text)
    def pos(w): return int((w-1)/total_weeks * n)
    s = pos(start); e = pos(end+1) if end<total_weeks else n
    return full_text[s:e]

def build_revision_pack(full_text:str, weeks_range:Tuple[int,int], level_hint:str, verbs:List[str], mcq_count:int=10, duration:int=20, diff:str="Medium")->dict:
    start,end = weeks_range
    rng_text = collect_weeks_text(full_text or "", start, end)
    if not rng_text.strip(): rng_text = full_text or "No source text available; please upload an e-book or paste text."
    summary = summarize_sentences(rng_text, k=8)
    keywords = extract_keywords(rng_text, topk=15)
    mid = (start+end)//2; implied_tier = policy_tier(mid)
...
def export_revision_docx(title:str, pack:dict)->Optional[bytes]:
    try:
        from docx import Document
    except Exception:
        return None
    ...
    if pack.get("keywords"):
        ...
        p = doc.add_paragraph(); p.add_run(", ".join(pack["keywords"]))
...
with tabs[4]:
    ...
    if scheme.startswith("3 tasks"):
        a_range = st.slider("Task A weeks", 1, 14, (1,3)); b_range = st.slider("Task B weeks", 1, 14, (4,7)); c_range = st.slider("Task C weeks", 1, 14, (8,14))
        ranges = [("Task A", a_range), ("Task B", b_range), ("Task C", c_range)]
    elif scheme.startswith("2 tasks"):
        a_range = st.slider("Task A weeks", 1, 14, (1,7)); b_range = st.slider("Task B weeks", 1, 14, (8,14))
        ranges = [("Task A", a_range), ("Task B", b_range)]
    else:
        a_range = st.slider("Task A weeks", 1, 14, (1,3)); b_range = st.slider("Task B weeks", 1, 14, (4,7))
        show_c = st.checkbox("Include Task C", value=True)
        if show_c:
            c_range = st.slider("Task C weeks", 1, 14, (8,14)); ranges = [("Task A", a_range), ("Task B", b_range), ("Task C", c_range)]
        else:
            ranges = [("Task A", a_range), ("Task B", b_range)]
    ...
    if make_btn:
        packs = []
        for label, rngw in ranges:
            pack = build_revision_pack(full_text, rngw, level_hint, verbs_pref, mcq_count=mcq_count, duration=duration, diff=diff)
            packs.append((label, rngw, pack))
        ...
        try:
            ...
            docx_ok = False
        for label, rngw, pack in packs:
            st.markdown(f"### {label} — Weeks {rngw[0]}–{rngw[1]}  ·  Tier: **{pack['tier']}**  ·  Focus: **{pack['focus']}**")
            with st.expander("Preview summary & key terms", expanded=False):
                if pack['summary']:
                    st.write("**Summary:**"); [st.write('- '+s) for s in pack['summary']]
                if pack['keywords']:
                    st.write("**Key terms:** " + ", ".join(pack['keywords']))
            st.write("**Quick MCQs (editable):**")
            key = f"rev_mcq_{label}"; pack['mcqs'] = st.data_editor(pack['mcqs'], hide_index=True, num_rows="dynamic", use_container_width=True, key=key)
            docx_bytes = export_revision_docx(f"{label} Revision — Weeks {rngw[0]}–{rngw[1]}", pack) if docx_ok else None
            today = dt.date.today().strftime("%Y-%m-%d"); base = f"{label.replace(' ','_')}_Weeks{rngw[0]}-{rngw[1]}_{today}"
            c1, c2 = st.columns(2)
            with c1:
