...
/* FORCE uploader dashed border: Streamlit uses either div or section for dropzone */
...
:where(div,section)[data-testid="stFileUploaderDropzone"]:hover{{
...
def edit_list(label, key, placeholder):
    ...
    if st.session_state.get(f"adding_{key}"):
        ...
        if csb.button("Cancel", key=f"cancel_{key}"): st.session_state[f"adding_{key}"] = False
    return selected
...
def generate_mcqs(n, verbs, source_text, include_answers=True):
    ...
    for i in range(n):
        ...
        distract=[f\"Unrelated detail about {random.choice(terms)}.\", f\"Common misconception about {term}.\", f\"Vague statement with {random.choice(terms)}.\"]
...
def main():
    ...
    with st.sidebar:
        ...
        # Immediate, visible 'uploaded' line
        upload_note_area = st.empty()
    ...
    low_exp  = st.expander(\"**Low (Weeks 1–4)** — Remember / Understand\", True)
    med_exp  = st.expander(\"**Medium (Weeks 5–9)** — Apply / Analyse\", False)
    high_exp = st.expander(\"**High (Weeks 10–14)** — Evaluate / Create\", False)
    ...
    # Parse upload once, show a single quiet status line below the uploader
    source_text = \"\"
    if uploaded_file is not None:
        ...
        source_text = text
        upload_note_area.markdown(f\"✅ **Uploaded:** {uploaded_file.name}  \n_Type:_ {ftype} — {note}\")
