...
with mcq_tab:
    ...
    if st.button("Generate MCQs"):
        ...
        for i,(stem,opts,ans) in enumerate(mcqs, start=1):
            ...
            # Per-question optional attachments (NOTE: do not pre-set uploader keys)
            passage_key = f"mcq_passage_{i}"
            img_key     = f"mcq_img_{i}"

            st.text_area(
                f"📄 Passage (optional) for Q{i}",
                value=st.session_state.get(passage_key, ""),
                key=passage_key,
                height=80,
            )
            st.file_uploader(
                f"🖼️ Image (optional) for Q{i}",
                type=["png","jpg","jpeg"],
                key=img_key,
            )
    ...
    def build_full_pack_docx(mcq_blocks, activities_list):
        ...
        if mcq_blocks:
            ...
            for idx, blk in enumerate(mcq_blocks, 1):
                ...
                if img is not None:
                    try:
                        img.seek(0); doc.add_picture(img, width=Inches(4.5))
