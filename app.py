# app.py — ADI Builder (polished styling + segmented selectors + progress)
# Run locally:
...
def _load_logo_data_uri():
    # Prefer assets/adi-logo.png; fall back to logo.png
    for p in ("assets/adi-logo.png", "logo.png"):
        if os.path.exists(p):
            with open(p, "rb") as f:
...
/* Cards */
.adi-card{background:var(--card); border:1px solid var(--border); border-radius:18px; padding:18px; box-shadow:var(--shadow); margin-bottom:16px}
...
/* Make radio dots ADI green */
input[type=radio]{ accent-color: var(--adi-green) !important }

/* Inputs → pill style; green glow on focus */
...
/* Streamlit BaseWeb selectbox clean look */
...
.stNumberInput > div{ border-radius:var(--radius-pill) !important; border:1px solid var(--border) !important; background:var(--adi-stone) !important }
.stNumberInput > div:focus-within{ border-color:var(--adi-green) !important; box-shadow:0 0 0 3px rgba(36,90,52,.25) !important }
...
with left:
    ...
    f = st.file_uploader("Drag and drop your file", type=["pdf", "docx", "pptx"])
    if f:
        # tiny, friendly progress (cosmetic)
        p = st.progress(0, text=f"Uploading {f.name}…")
        for i in range(0, 101, 12):
            time.sleep(0.02)
            p.progress(i, text=f"Uploading {f.name}…")
        p.empty()
        st.success(f"Uploaded **{f.name}** ({f.size/1e6:.2f} MB)")
    ...
    # Activity params + Bloom highlight
    st.markdown('<div class="adi-card">', unsafe_allow_html=True)
