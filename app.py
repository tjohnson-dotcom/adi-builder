# app.py — ADI Learning Tracker Question Generator (simple 4-step flow)
...
:root{
  ...
  --card:#ffffff;
  --border:#e7ecea;
  --shadow:0 6px 28px rgba(36,90,52,.08);
}
...
/* ---------- Header ---------- */
.h1{
  font-size:28px; font-weight:900; color:var(--ink); letter-spacing:.1px; margin:0 0 2px 0;
}
.small{ color:var(--muted); }

/* thin rule matching brand */
hr{ border:none; height:1px; background:linear-gradient(90deg, rgba(36,90,52,.25), rgba(36,90,52,.06)); margin:.9rem 0 1.1rem; }
...
/* ---------- Callouts ---------- */
.notice{
  padding:14px; border-radius:14px; border:1px dashed #dae4de; background:#fff;
}
.callout{
  padding:14px; border-radius:14px; background:#eef6ff; border:1px solid #dbeafe; margin:10px 0;
}
.next-tip{
  margin-top:14px; padding:12px 14px; border-radius:12px;
  background:linear-gradient(180deg,#f4faf6,#f7fbf8);
  border:1px solid #dfece6; color:#0b3d22;
}

/* ---------- Tabs: thicker underline + spacing ---------- */
.stTabs [role="tablist"] { gap: .5rem; }
.stTabs [role="tab"] { font-weight:800; color:#3b3b3b; padding: .6rem .8rem; border-radius:10px 10px 0 0; }
...
.stButton>button{
  ...
  border-radius:12px; padding:.6rem 1rem; box-shadow:0 6px 18px rgba(36,90,52,.20);
}
.stButton>button:hover{ filter: brightness(1.05); }
.stButton>button:focus{ outline:3px solid rgba(36,90,52,.25); }
...
/* ---------- Uploads ---------- */
[data-testid="stFileUploader"] .st-emotion-cache-1f3916k,  /* Streamlit internal: outer box */
...
.badge{
  ...
  width:26px; height:26px; border-radius:999px; color:#fff; font-weight:800; font-size:12px; margin-right:10px;
}
...
with tab1:
    ...
    st.markdown(
        "<div class='next-tip'>Next: go to <b>② Generate</b> to create questions & activities.</div>",
