# -*- coding: utf-8 -*-
# ADI Builder — Clean ADI (radio-only Bloom, robust upload feedback, exports)
...
# ------------------------ Theme (safe string formatting) ------------------------
ADI_GREEN = "#245a34"; ADI_GREEN_DARK="#1a4426"; ADI_GOLD="#C8A85A"; ADI_STONE="#f4f4f2"
CSS = """
<style>
:root { --adi-green:%(adi_green)s; --adi-green-dark:%(adi_green_dark)s; --adi-gold:%(adi_gold)s; --adi-stone:%(adi_stone)s; }
...
/* Export banner */
.adi-banner { display:block; background:#ffffff; border-left:6px solid var(--adi-gold); color:#1f2937; font-weight:900;
              letter-spacing:.04em; text-transform:uppercase; padding:8px 16px; border-radius:8px; margin:0 auto 10px auto;
              width:max-content; box-shadow:0 2px 8px rgba(0,0,0,.06); }
</style>
""" % {"adi_green": ADI_GREEN, "adi_green_dark": ADI_GREEN_DARK, "adi_gold": ADI_GOLD, "adi_stone": ADI_STONE}
...
with tabs[0]:
    ...
    if up is not None:
        label = up.name
        size = f" · {getattr(up, 'size', 0)/1e6:.1f} MB"
...
with tabs[1]:
    ...
    st.markdown(
        "Policy: <b>%s</b> · Selected: <b class='%s'>%s</b>%s" % (
            current_policy, ("ok" if selected_tier==current_policy else "warn"), selected_tier,
            (" ✓" if selected_tier==current_policy else " (mismatch)")
        ),
...
with tabs[2]:
    ...
    st.write("")
    st.markdown("**Quick Editor**"); st.caption("Edit inline. Your exports will use this exact table.")
...
with tabs[3]:
    ...
    with c4:
        if st.session_state.get("activities") and Document:
            st.download_button("Export · Activities DOCX", data=activities_docx(st.session_state["activities"]),
